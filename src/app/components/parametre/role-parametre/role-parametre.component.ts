...
import { RoleInsertionComponent } from '../role-dialog/role-insertion/role-insertion.component';
...
@Component({
  ...
  templateUrl: './role-parametre.component.html',
  styleUrl: './role-parametre.component.css'
})
export class RoleParametreComponent  implements OnInit{
  ...
  loadRoles() {
    ...
    this.apiService.getWithData('api/roles/search',this.searchCriteria).then(
      (response) => {
  ...
  changeStatut(id: string, isChecked: boolean) {
    ...
    this.apiService.insert('api/role/' + id, data).then(
      (response) => {
        this.loadRoles();
        this.loader = false;
      },
  ...
  openMotriciteDialog() {
    const dialogRef = this.dialog.open(RoleInsertionComponent, {
      width: '800px',
      disableClose: true
    });
